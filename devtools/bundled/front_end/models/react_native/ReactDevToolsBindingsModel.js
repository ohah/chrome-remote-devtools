...
export class ReactDevToolsBindingsModel extends SDK.SDKModel.SDKModel {
    ...
    onExecutionContextCreated({ data: executionContext }) {
        ...
        void this.waitForFuseboxDispatcherToBeInitialized()
            .then(() => {
            this.dispatchEventToListeners("BackendExecutionContextCreated" /* Events.BACKEND_EXECUTION_CONTEXT_CREATED */);
            this.flushOutDomainMessagesQueues();
        })
            .catch((error) => this.dispatchEventToListeners("BackendExecutionContextUnavailable" /* Events.BACKEND_EXECUTION_CONTEXT_UNAVAILABLE */, error.message));
    }
    onExecutionContextDestroyed({ data: executionContext }) {
        ...
        this.dispatchEventToListeners("BackendExecutionContextDestroyed" /* Events.BACKEND_EXECUTION_CONTEXT_DESTROYED */);
    }

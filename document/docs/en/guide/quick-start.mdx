...
If you omit `deviceId`, a random UUID is generated each time (the same device may appear as a new entry after app restart). For a **stable device identifier** in the Inspector list, install [react-native-device-info](https://www.npmjs.com/package/react-native-device-info) and pass `getUniqueId()`:
...
function App() {
  ...
  useEffect(() => {
    getUniqueId()
      .then(setDeviceId)
      .catch((err) => {
        console.warn('getUniqueId failed, Inspector will use random UUID:', err);
        setDeviceId('');
      });
  }, []);
...
- Omit `deviceId` (or pass `undefined`) to use a random UUID for the session.
- Use `react-native-device-info` and pass `getUniqueId()` so the same device appears in the Inspector list across app restarts.

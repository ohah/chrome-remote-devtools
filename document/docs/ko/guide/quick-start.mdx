...
`deviceId`를 생략하면 매번 랜덤 UUID가 생성됩니다(앱 재시작 후 같은 기기가 새 항목으로 보일 수 있음). Inspector 목록에서 **동일 기기 식별자**를 유지하려면 [react-native-device-info](https://www.npmjs.com/package/react-native-device-info)를 설치해 `getUniqueId()`를 넘기세요.
...
function App() {
  ...
  useEffect(() => {
    getUniqueId()
      .then(setDeviceId)
      .catch((err) => {
        console.warn('getUniqueId 실패, Inspector는 랜덤 UUID 사용:', err);
        setDeviceId('');
      });
  }, []);
...
- `deviceId`를 생략하거나 `undefined`로 두면 해당 세션에는 랜덤 UUID가 사용됩니다.
- `react-native-device-info`의 `getUniqueId()`를 넘기면 앱 재시작 후에도 Inspector 목록에서 같은 기기로 표시됩니다.

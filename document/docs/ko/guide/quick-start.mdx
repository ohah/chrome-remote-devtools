...
더 친절한 소개와 예제 앱은 [`@ohah/chrome-remote-devtools-inspector-react-native` README](https://github.com/ohah/chrome-remote-devtools/tree/main/packages/react-native-inspector)를 참고하세요.
...
`deviceId`는 **필수**이며 Tauri/Inspector 연결 목록에 표시됩니다. 앱 재시작 후에도 **동일 기기 식별자**를 유지하려면 [react-native-device-info](https://www.npmjs.com/package/react-native-device-info)를 설치해 `getUniqueId()`를 넘기세요.
...
Provider 사용 시: `getUniqueId()` 등으로 얻은 `deviceId`를 넘기면 Inspector 목록에서 같은 기기가 일관되게 표시됩니다.

#### 사용 예제: Provider + 안정적 device ID

`getUniqueId()`를 받은 뒤 Provider를 렌더링하는 예제 ([예제 앱](https://github.com/ohah/chrome-remote-devtools/tree/main/examples/react-native)과 동일한 패턴):
...
- `deviceId`는 필수이며, `react-native-device-info`의 `getUniqueId()`를 넘기면 앱 재시작 후에도 Inspector 목록에서 같은 기기로 표시됩니다.

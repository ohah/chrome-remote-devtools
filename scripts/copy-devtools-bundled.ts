...
const devtoolsBuildPath = resolve(__dirname, "../devtools/devtools-frontend/out/Default/gen/front_end");
...
if (!statSync(devtoolsBuildPath).isDirectory()) {
  ...
  console.error("Please run 'gclient sync' and 'npm run build' in devtools/devtools-frontend first");
  process.exit(1);
}

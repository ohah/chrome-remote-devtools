// Network domain integration tests / Network 도메인 통합 테스트
import { test, expect } from './fixtures/server';

test.describe('Network Domain Integration', () => {
  ...
  test('should get server endpoints / 서버 엔드포인트 가져오기', async ({ page, serverUrl }) => {
    const response = await page.request.get(`${serverUrl}/json/inspectors`);
